...
const Upload = () => {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const response = await fetch('https://lyncitapplications.xyz:8086/files/uploadfile/', {
        method: 'POST',
        body: formData,
        credentials: 'include',
        headers: {
    ...
    if (file) {
      if (isValidFileType(file)) {
        try {
          ...
          const { guid } = await handleFileUpload(file);
          // Now we can use the GUID to test download
          console.log(`File saved with GUID: ${guid}`);
          ...
          console.error('Error during upload/download:', error.message);
          setUploadedFile("");
          setFileError(true);
        }
      } else {
    ...
    if (isUploading) {
      return (
        <div className="flex flex-col justify-center items-center">
          <div className="mb-3">
            {/* You can add a loading spinner SVG here */}
          </div>
          <p className="text-[#637083]">Uploading...</p>
        </div>
      );
    }
